...
enum bool_type {
   ...
   bool_type_all_types = BITFIELD_BIT(3) - 1,
};
...
static inline bool
block_is_loop_header(nir_block *block)
{
...
static bool
phi_to_bool(nir_builder *b, nir_phi_instr *phi)
{
...
nir_opt_phi_to_bool(nir_shader *shader)
{
   ...
   nir_foreach_function_impl(impl, shader) {
      nir_foreach_block(block, impl) {
         bool is_loop_header = block_is_loop_header(block);
         nir_foreach_instr(instr, block) {
            if (instr->type == nir_instr_type_phi && is_loop_header) {
               nir_foreach_phi_src(phi_src, nir_instr_as_phi(instr)) {
                  instr->pass_flags = src_pass_flags(&phi_src->src);
                  /* We only know the types of the first phi source
   ...
   nir_foreach_function_impl(impl, shader) {
      ...
      nir_foreach_block(block, impl) {
         nir_foreach_phi(phi, block) {
            impl_progress |= phi_to_bool(&b, phi);
         }
      }
