...
#define perf_query_renderdoc_perfcntr_iova(pool, query, field, i)          \
   pool->bo->iova + pool->query_stride * (query) +                         \
   sizeof(struct query_slot) +                                             \
   sizeof(uint64_t) * pool->perf_query.renderdoc.counter_index_count +     \
   sizeof(struct perfcntr_query_slot) * (i) +                              \
   offsetof(struct perfcntr_query_slot, field)

#define perf_query_renderdoc_perfcntr_addr(pool, query, field, i)          \
   (uint64_t *) ((char *) pool->bo->map + pool->query_stride * (query) +   \
                 sizeof(struct query_slot) +                                \
                 sizeof(uint64_t) * pool->perf_query.renderdoc.counter_index_count + \
                 sizeof(struct perfcntr_query_slot) * (i) +                 \
...
static bool
is_perf_query_generic(struct tu_query_pool *pool)
{
   return pool->vk.query_type == VK_QUERY_TYPE_PERFORMANCE_QUERY_KHR &&
          pool->perf_query_type == TU_PERF_QUERY_TYPE_GENERIC;
}

static bool
is_perf_query_renderdoc(struct tu_query_pool *pool)
{
   return pool->vk.query_type == VK_QUERY_TYPE_PERFORMANCE_QUERY_KHR &&
          pool->perf_query_type == TU_PERF_QUERY_TYPE_RENDERDOC;
}
                   ...
                   VkQueryPool *pQueryPool)
{
   ...
   case VK_QUERY_TYPE_PERFORMANCE_QUERY_KHR: {
      ...
      if (TU_DEBUG(RENDERDOC)) {
         perf_query_type = TU_PERF_QUERY_TYPE_RENDERDOC;
   ...
   if (is_perf_query_generic(pool)) {
      struct tu_perf_query_generic *perf_query = &pool->perf_query.generic;
   ...
   if (is_perf_query_renderdoc(pool)) {
      struct tu_perf_query_renderdoc *perf_query = &pool->perf_query.renderdoc;
      perf_query->counter_index_count = perf_query_info->counterIndexCount;
...
get_result_count(struct tu_query_pool *pool)
{
   ...
   case VK_QUERY_TYPE_PERFORMANCE_QUERY_KHR:
      assert(is_perf_query_generic(pool) ^ is_perf_query_renderdoc(pool));
      if (is_perf_query_renderdoc(pool))
         return pool->perf_query.renderdoc.counter_index_count;
      return pool->perf_query.generic.counter_index_count;
   default:
   ...
   for (uint32_t i = 0; i < queryCount; i++) {
      ...
      for (uint32_t k = 0; k < result_count; k++) {
         if (available) {
            ...
            } else if (is_perf_query_generic(pool)) {
            ...
            if (is_perf_query_generic(pool)) {
               struct tu_perf_query_generic *perf_query = &pool->perf_query.generic;
               struct tu_perf_query_data *data = &perf_query->data[k];
               VkPerformanceCounterStorageKHR storage =
            ...
            } else if (is_perf_query_renderdoc(pool)) {
               struct tu_perf_query_renderdoc *perf_query = &pool->perf_query.renderdoc;
               ...
               for (unsigned l = 0; l < derived_counter->num_perfcntrs; ++l) {
                  ...
                  uint64_t *perfcntr_result = perf_query_renderdoc_perfcntr_addr(pool, query, result, perfcntr_map);
               ...
               *result = derived_counter->derive(perfcntr_values);
               write_performance_query_value_cpu(result_base, k, storage, result);
            } else {
   ...
   for (uint32_t i = 0; i < queryCount; i++) {
      ...
      if (is_perf_query_renderdoc(pool)) {
         ...
         for (uint32_t j = 0; j < perf_query->collection->num_enabled_perfcntrs; ++j) {
            uint64_t perfcntr_result_iova = perf_query_renderdoc_perfcntr_iova(pool, query, result, j);
...
template <chip CHIP>
static void
emit_begin_perf_query_generic(struct tu_cmd_buffer *cmdbuf,
                              struct tu_query_pool *pool,
                              uint32_t query)
{
   struct tu_cs *cs = cmdbuf->state.pass ? &cmdbuf->draw_cs : &cmdbuf->cs;
   ...
   for (uint32_t i = 0; i < perf_query->counter_index_count; i++) {
      struct tu_perf_query_data *data = &perf_query->data[i];
...
template <chip CHIP>
static void
emit_begin_perf_query_renderdoc(struct tu_cmd_buffer *cmdbuf,
                                struct tu_query_pool *pool,
                                uint32_t query)
{
   struct tu_cs *cs = cmdbuf->state.pass ? &cmdbuf->draw_cs : &cmdbuf->cs;
   ...
   for (uint32_t i = 0; i < perf_query->collection->num_enabled_perfcntrs; ++i) {
      ...
      const struct fd_perfcntr_countable *countable = perf_query->collection->enabled_perfcntrs[i].countable;
   ...
   for (uint32_t i = 0; i < perf_query->collection->num_enabled_perfcntrs; ++i) {
      const struct fd_perfcntr_counter *counter = perf_query->collection->enabled_perfcntrs[i].counter;
      uint64_t begin_iova = perf_query_renderdoc_perfcntr_iova(pool, query, begin, i);
                           ...
                           uint32_t index)
{
   ...
   case VK_QUERY_TYPE_PERFORMANCE_QUERY_KHR:
      if (is_perf_query_generic(pool))
         emit_begin_perf_query_generic<CHIP>(cmdbuf, pool, query);
      if (is_perf_query_renderdoc(pool))
         emit_begin_perf_query_renderdoc<CHIP>(cmdbuf, pool, query);
      break;
   case VK_QUERY_TYPE_PIPELINE_STATISTICS:
...
template <chip CHIP>
static void
emit_end_perf_query_generic(struct tu_cmd_buffer *cmdbuf,
                            struct tu_query_pool *pool,
                            uint32_t query)
{
   struct tu_cs *cs = cmdbuf->state.pass ? &cmdbuf->draw_cs : &cmdbuf->cs;
...
template <chip CHIP>
static void
emit_end_perf_query_renderdoc(struct tu_cmd_buffer *cmdbuf,
                              struct tu_query_pool *pool,
                              uint32_t query)
{
   struct tu_cs *cs = cmdbuf->state.pass ? &cmdbuf->draw_cs : &cmdbuf->cs;
   ...
   for (uint32_t i = 0; i < perf_query->collection->num_enabled_perfcntrs; ++i) {
      const struct fd_perfcntr_counter *counter = perf_query->collection->enabled_perfcntrs[i].counter;
      uint64_t end_iova = perf_query_renderdoc_perfcntr_iova(pool, query, end, i);
   ...
   for (uint32_t i = 0; i < perf_query->collection->num_enabled_perfcntrs; ++i) {
      uint64_t result_iova = perf_query_renderdoc_perfcntr_iova(pool, query, result, i);
      uint64_t begin_iova = perf_query_renderdoc_perfcntr_iova(pool, query, begin, i);
      uint64_t end_iova = perf_query_renderdoc_perfcntr_iova(pool, query, end, i);
                         ...
                         uint32_t index)
{
   ...
   case VK_QUERY_TYPE_PERFORMANCE_QUERY_KHR:
      if (is_perf_query_generic(pool))
         emit_end_perf_query_generic<CHIP>(cmdbuf, pool, query);
      if (is_perf_query_renderdoc(pool))
         emit_end_perf_query_renderdoc<CHIP>(cmdbuf, pool, query);
      break;
   case VK_QUERY_TYPE_PIPELINE_STATISTICS:
