...
class Field(object):
    ...
    def emit_template_struct(self, dim):
        if self.type == 'address':
            ...
            type = 'bool'
        elif self.type in ['float', 'half', 'ushort', 'ulod', 'slod']:
            type = 'float'
        elif self.type in ['uint', 'hex'] and self.end - self.start > 32:
            type = 'uint64_t'
        elif self.type == 'int':
            type = 'int32_t'
        elif self.type in ['uint', 'hex', 'mipaddress']:
            type = 'uint32_t'
        ...
        for index in range(math.ceil(self.length / 4)):
            ...
            for contributor in word.contributors:
                ...
                if pack_as_uint:
                    ...
                    s = "__gen_pack_ushort(%s, %d, %d)" % (value, start, end)
                elif field.type == "mipaddress":
                    assert(end - start + 1 == 26)
                    s = "__gen_pack_mipaddress(%s, %d, %d)" % (value, start, end)
    ...
    def emit_unpack_function(self):
        ...
        for fieldref in fieldrefs:
            ...
            if field.type in set(["uint", "address", "hex"]) | self.parser.enums:
                ...
                convert = "__gen_unpack_ushort"
            elif field.type == "mipaddress":
                convert = "__gen_unpack_mipaddress"
            elif field.type == "ulod":
