...
static inline uint64_t
__gen_pack_ushort(float f, uint32_t start, uint32_t end)
{
   return util_bitpack_uint(float_to_ushort(f), start, end);
}

static inline float
__gen_unpack_ushort(CONSTANT_ uint32_t *restrict cl, uint32_t start, uint32_t end)
{
...
/*
 * Only the top 26 bit of a mip address is stored.
 */
static inline uint64_t
__gen_pack_mipaddress(uint32_t u32, uint32_t start, uint32_t end)
{
   assert((u32 & 0x3f) == 0);
   return util_bitpack_uint(u32 >> 6, start, end);
}

static inline uint64_t
__gen_unpack_mipaddress(CONSTANT_ uint32_t *restrict cl, uint32_t start, uint32_t end)
{
   return __gen_unpack_uint(cl, start, end) << 6;
}
