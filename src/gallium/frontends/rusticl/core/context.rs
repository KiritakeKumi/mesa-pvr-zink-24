...
impl Context {
    ...
    pub unsafe fn alloc_svm_ptr(
        ...
        let layout = unsafe {
            Layout::from_size_align_unchecked(size.get() as usize, alignment.get() as usize)
        };
    ...
    pub fn add_bda_ptr(&self, buffer: &Arc<Buffer>) {
        if let Some(iter) = buffer.get_address_of_res() {
    ...
    pub fn remove_bda(&self, buf: &Buffer) {
        ...
        for (dev, bdas) in bda_ptrs.iter_mut() {
            if let Some(address) = buf.vm_address(dev) {
