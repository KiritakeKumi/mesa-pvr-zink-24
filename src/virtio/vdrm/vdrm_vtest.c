/*
 * Copyright © 2023 Google, Inc.
 * SPDX-License-Identifier: MIT
 */

#define VIRGL_RENDERER_UNSTABLE_APIS 1

#include <fcntl.h>
#include <string.h>
#include <sys/mman.h>
#include <sys/socket.h>
#include <sys/un.h>
#include <xf86drm.h>

#include "vdrm.h"

#include "drm-uapi/virtgpu_drm.h"
#include "util/os_file.h"
#include "util/libsync.h"
#include "util/log.h"
#include "util/os_time.h"
#include "util/perf/cpu_trace.h"
#include "util/u_dynarray.h"
#include "util/u_idalloc.h"
#include "util/u_process.h"
#include "util/u_sync_provider.h"

#include "vtest/vtest_protocol.h"

#define SHMEM_SZ 0x4000

struct vtest_device {
   struct vdrm_device base;
   int sock_fd;
   simple_mtx_t lock;
   unsigned protocol_version;
   uint32_t shmem_res_id;

   /* used for allocating indexes into bo_table: */
   struct util_idalloc bo_idx_allocator;

   /* table of BO info: */
   struct util_dynarray bo_table;
};
DEFINE_CAST(vdrm_device, vtest_device)

struct vtest_bo {
   uint32_t res_id;
};

/*
 * Our fake handles are just indexes into the bo table plus one
 * (because zero is an invalid handle):
 */
static uint32_t idx2handle(uint32_t idx) { return idx + 1; }
static uint32_t handle2idx(uint32_t handle) { return handle - 1; }

static struct vtest_bo *
handle2bo(struct vtest_device *vtdev, uint32_t handle)
{
   simple_mtx_assert_locked(&vtdev->lock);
   return util_dynarray_element(&vtdev->bo_table, struct vtest_bo, handle2idx(handle));
}

static int
vtest_write(struct vtest_device *vtdev, const void *buf, int size)
{
   simple_mtx_assert_locked(&vtdev->lock);
   const void *ptr = buf;
   int left;
   int ret;
   left = size;
   do {
      ret = write(vtdev->sock_fd, ptr, left);
      if (ret < 0)
         return -errno;
      left -= ret;
      ptr += ret;
   } while (left);
   return size;
}

static int
vtest_read_fd(int fd, void *buf, int size)
{
   void *ptr = buf;
   int left;
   int ret;
   left = size;
   do {
      ret = read(fd, ptr, left);
      if (ret <= 0) {
         mesa_loge("lost connection to rendering server on %d read %d %d",
                   size, ret, errno);
         abort();
         return ret < 0 ? -errno : 0;
      }
      left -= ret;
      ptr += ret;
   } while (left);
   return size;
}

static int
vtest_read(struct vtest_device *vtdev, void *buf, int size)
{
   simple_mtx_assert_locked(&vtdev->lock);
   return vtest_read_fd(vtdev->sock_fd, buf, size);
}

static int
vtest_send_fd(struct vtest_device *vtdev, int fd)
{
   simple_mtx_assert_locked(&vtdev->lock);
   struct iovec iovec;
   char buf[CMSG_SPACE(sizeof(int))];
   char c = 0;
   struct msghdr msgh = { 0 };
   memset(buf, 0, sizeof(buf));

   iovec.iov_base = &c;
   iovec.iov_len = sizeof(char);

   msgh.msg_name = NULL;
   msgh.msg_namelen = 0;
   msgh.msg_iov = &iovec;
   msgh.msg_iovlen = 1;
   msgh.msg_control = buf;
   msgh.msg_controllen = sizeof(buf);
   msgh.msg_flags = 0;

   struct cmsghdr *cmsg = CMSG_FIRSTHDR(&msgh);
   cmsg->cmsg_level = SOL_SOCKET;
   cmsg->cmsg_type = SCM_RIGHTS;
   cmsg->cmsg_len = CMSG_LEN(sizeof(int));

   *((int *) CMSG_DATA(cmsg)) = fd;

   int size = sendmsg(vtdev->sock_fd, &msgh, 0);
   if (size < 0) {
      mesa_loge("Failed to send fd");
      return -1;
   }

   return 0;
}

static int
vtest_receive_fd(struct vtest_device *vtdev)
{
   simple_mtx_assert_locked(&vtdev->lock);
   struct cmsghdr *cmsgh;
   struct msghdr msgh = { 0 };
   char buf[CMSG_SPACE(sizeof(int))], c;
   struct iovec iovec;

   iovec.iov_base = &c;
   iovec.iov_len = sizeof(char);

   msgh.msg_name = NULL;
   msgh.msg_namelen = 0;
   msgh.msg_iov = &iovec;
   msgh.msg_iovlen = 1;
   msgh.msg_control = buf;
   msgh.msg_controllen = sizeof(buf);
   msgh.msg_flags = 0;

   int size = recvmsg(vtdev->sock_fd, &msgh, 0);
   if (size < 0) {
     mesa_loge("Failed with %s", strerror(errno));
     return -1;
   }

   cmsgh = CMSG_FIRSTHDR(&msgh);
   if (!cmsgh) {
     mesa_loge("No headers available");
     return -1;
   }

   if (cmsgh->cmsg_level != SOL_SOCKET) {
     mesa_loge("invalid cmsg_level %d", cmsgh->cmsg_level);
     return -1;
   }

   if (cmsgh->cmsg_type != SCM_RIGHTS) {
     mesa_loge("invalid cmsg_type %d", cmsgh->cmsg_type);
     return -1;
   }

   return *((int *) CMSG_DATA(cmsgh));
}

static uint32_t
vtest_create_blob(struct vtest_device *v, size_t size, uint32_t flags,
                  uint64_t blob_id, int *out_fd)
{
   uint32_t vtest_hdr[VTEST_HDR_SIZE];
   uint32_t vcmd_res_create_blob[VCMD_RES_CREATE_BLOB_SIZE];
   uint32_t res_id = 0;
   /* If not mappable/shareable/exportable we don't need to keep the fd: */
   bool close_fd = !flags;

   MESA_TRACE_FUNC();

   size = ALIGN_POT(size, getpagesize());

   vtest_hdr[VTEST_CMD_LEN] = VCMD_RES_CREATE_BLOB_SIZE;
   vtest_hdr[VTEST_CMD_ID] = VCMD_RESOURCE_CREATE_BLOB;

   vcmd_res_create_blob[VCMD_RES_CREATE_BLOB_TYPE] = VCMD_BLOB_TYPE_HOST3D;

   /* Hack: vtest tries to send us an fd whether we want it or not.. which
    * fails if it is not marked as exportable in some form:
    */
   flags &= ~VIRTGPU_BLOB_FLAG_USE_SHAREABLE;
   flags |= VIRTGPU_BLOB_FLAG_USE_MAPPABLE;

   vcmd_res_create_blob[VCMD_RES_CREATE_BLOB_FLAGS] = flags;
   vcmd_res_create_blob[VCMD_RES_CREATE_BLOB_SIZE_LO] = (uint32_t)size;
   vcmd_res_create_blob[VCMD_RES_CREATE_BLOB_SIZE_HI] =
      (uint32_t)(size >> 32);
   vcmd_res_create_blob[VCMD_RES_CREATE_BLOB_ID_LO] = (uint32_t)blob_id;
   vcmd_res_create_blob[VCMD_RES_CREATE_BLOB_ID_HI] =
      (uint32_t)(blob_id >> 32);

   vtest_write(v, vtest_hdr, sizeof(vtest_hdr));
   vtest_write(v, vcmd_res_create_blob, sizeof(vcmd_res_create_blob));

   vtest_read(v, vtest_hdr, sizeof(vtest_hdr));
   assert(vtest_hdr[VTEST_CMD_LEN] == 1);
   assert(vtest_hdr[VTEST_CMD_ID] == VCMD_RESOURCE_CREATE_BLOB);

   // TODO we'd prefer to allocate the res_id on client side to avoid sync
   // round trip:
   vtest_read(v, &res_id, sizeof(uint32_t));

   // TODO we'd prefer to not get an fd back if we don't need to mmap or share
   // if we git rid of this and res_id readback, then we avoid the extra round
   // trip.. fd can be requested via VCMD_RESOURCE_EXPORT_FD
   *out_fd = vtest_receive_fd(v);

   if (close_fd) {
      close(*out_fd);
      *out_fd = -1;
   }

   return res_id;
}

static void
close_res_id(struct vtest_device *vtdev, uint32_t res_id)
{
   uint32_t vtest_hdr[VTEST_HDR_SIZE];
   uint32_t cmd[1];
   vtest_hdr[VTEST_CMD_LEN] = 1;
   vtest_hdr[VTEST_CMD_ID] = VCMD_RESOURCE_UNREF;

   MESA_TRACE_FUNC();

   cmd[0] = res_id;
   vtest_write(vtdev, &vtest_hdr, sizeof(vtest_hdr));
   vtest_write(vtdev, &cmd, sizeof(cmd));
}

static void
vtest_submit_cmd(struct vtest_device *vtdev,
                 const void *cmd, uint32_t cmdsize,
                 enum vcmd_submit_cmd2_flag flags,
                 uint32_t ring_idx, uintptr_t *fencep,
                 uint32_t num_in_syncobjs,
                 uint32_t num_out_syncobjs)
{
   simple_mtx_assert_locked(&vtdev->lock);
   MESA_TRACE_FUNC();

   const size_t header_size = sizeof(uint32_t) + sizeof(struct vcmd_submit_cmd2_batch);

   uint32_t vtest_hdr[VTEST_HDR_SIZE];
   vtest_hdr[VTEST_CMD_LEN] = (header_size + cmdsize) / sizeof(uint32_t);
   vtest_hdr[VTEST_CMD_ID] = VCMD_SUBMIT_CMD2;
   vtest_write(vtdev, vtest_hdr, sizeof(vtest_hdr));

   const uint32_t batch_count = 1;
   vtest_write(vtdev, &batch_count, sizeof(batch_count));

   struct vcmd_submit_cmd2_batch dst = {
      .cmd_offset = header_size / sizeof(uint32_t),
      .cmd_size = cmdsize / sizeof(uint32_t),
      .flags = flags,
      .ring_idx = ring_idx,
      .num_in_syncobj = num_in_syncobjs,
      .num_out_syncobj = num_out_syncobjs,
   };
   vtest_write(vtdev, &dst, sizeof(dst));

   vtest_write(vtdev, cmd, cmdsize);

   // TODO if fencep!=NULL then we need a way to get a fence that
   // can be later waited on..
}

static int
vtest_execbuf_locked(struct vdrm_device *vdev, struct vdrm_execbuf_params *p,
                     void *command, unsigned size)
{
   struct vtest_device *vtdev = to_vtest_device(vdev);
   enum vcmd_submit_cmd2_flag flags = VCMD_SUBMIT_CMD2_FLAG_RING_IDX;

   MESA_TRACE_FUNC();

   if (p->has_in_fence_fd)
      flags |= VCMD_SUBMIT_CMD2_FLAG_IN_FENCE_FD;

   if (p->needs_out_fence_fd)
      flags |= VCMD_SUBMIT_CMD2_FLAG_OUT_FENCE_FD;

   // TODO deal with handles for implicit sync?

   simple_mtx_assert_locked(&vdev->eb_lock);

   simple_mtx_lock(&vtdev->lock);
   vtest_submit_cmd(vtdev, command, size, flags, p->ring_idx, NULL,
                    p->num_in_syncobjs, p->num_out_syncobjs);

   if (p->num_in_syncobjs)
      vtest_write(vtdev, p->in_syncobjs, sizeof(*p->in_syncobjs) * p->num_in_syncobjs);
   if (p->num_out_syncobjs)
      vtest_write(vtdev, p->out_syncobjs, sizeof(*p->out_syncobjs) * p->num_out_syncobjs);

   if (p->has_in_fence_fd)
      vtest_send_fd(vtdev, p->fence_fd);
   if (p->needs_out_fence_fd)
      p->fence_fd = vtest_receive_fd(vtdev);

   simple_mtx_unlock(&vtdev->lock);

   return 0;
}

static int
vtest_flush_locked(struct vdrm_device *vdev, uintptr_t *fencep)
{
   struct vtest_device *vtdev = to_vtest_device(vdev);

   simple_mtx_assert_locked(&vdev->eb_lock);

   if (!vdev->reqbuf_len)
      return 0;

   simple_mtx_lock(&vtdev->lock);
   vtest_submit_cmd(vtdev, vdev->reqbuf, vdev->reqbuf_len,
                    VCMD_SUBMIT_CMD2_FLAG_RING_IDX, 0,
                    fencep, 0, 0);
   simple_mtx_unlock(&vtdev->lock);

   vdev->reqbuf_len = 0;
   vdev->reqbuf_cnt = 0;

   return 0;
}

static void
vtest_wait_fence(struct vdrm_device *vdev, uintptr_t fence)
{
   MESA_TRACE_FUNC();
   // TODO
}

/**
 * Note, does _not_ de-duplicate handles
 */
static uint32_t
vtest_dmabuf_to_handle(struct vdrm_device *vdev, int fd)
{
   unreachable("unimplemented");
   return 0;
}

static uint32_t
vtest_handle_to_res_id(struct vdrm_device *vdev, uint32_t handle)
{
   struct vtest_device *vtdev = to_vtest_device(vdev);

   MESA_TRACE_FUNC();

   simple_mtx_lock(&vtdev->lock);
   struct vtest_bo *vbo = handle2bo(vtdev, handle);
   uint32_t ret = vbo->res_id;
   simple_mtx_unlock(&vtdev->lock);

   return ret;
}

static uint32_t
vtest_bo_create(struct vdrm_device *vdev, size_t size, uint32_t blob_flags,
                  uint64_t blob_id, struct vdrm_ccmd_req *req)
{
   struct vtest_device *vtdev = to_vtest_device(vdev);
   uint32_t res_id;
   uint32_t flags = 0;
   int fd;

   MESA_TRACE_FUNC();

   if (blob_flags & VIRTGPU_BLOB_FLAG_USE_MAPPABLE)
      flags |= VCMD_BLOB_FLAG_MAPPABLE;

   if (blob_flags & VIRTGPU_BLOB_FLAG_USE_SHAREABLE)
      flags |= VCMD_BLOB_FLAG_SHAREABLE;

   if (blob_flags & VIRTGPU_BLOB_FLAG_USE_CROSS_DEVICE)
      flags |= VCMD_BLOB_FLAG_CROSS_DEVICE;

   simple_mtx_lock(&vtdev->lock);
   if (req) {
      vtest_submit_cmd(vtdev, req, req->len,
                       VCMD_SUBMIT_CMD2_FLAG_RING_IDX, 0,
                       NULL, 0, 0);
   }

   res_id = vtest_create_blob(vtdev, size, flags, blob_id, &fd);
   if (!res_id)
      goto out_unlock;

   close(fd);

   uint32_t idx = util_idalloc_alloc(&vtdev->bo_idx_allocator);

   /* Ensure sufficient table size: */
   if (!util_dynarray_resize(&vtdev->bo_table, struct vtest_bo, idx + 1)) {
      close_res_id(vtdev, res_id);
      res_id = 0;
      goto out_unlock;
   }

   struct vtest_bo *vbo = handle2bo(vtdev, idx2handle(idx));

   vbo->res_id = res_id;

out_unlock:
   simple_mtx_unlock(&vtdev->lock);

   if (!res_id)
      return 0;

   return idx2handle(idx);
}

static int
vtest_bo_wait(struct vdrm_device *vdev, uint32_t handle)
{
   MESA_TRACE_FUNC();
   // TODO implicit sync
   return 0;
}

static int vtest_bo_export_dmabuf(struct vdrm_device *vdev, uint32_t handle);

static void *
vtest_bo_map(struct vdrm_device *vdev, uint32_t handle, size_t size, void *placed_addr)
{
   MESA_TRACE_FUNC();

   int fd = vtest_bo_export_dmabuf(vdev, handle);
   void *ret = mmap(placed_addr, size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
   close(fd);

   return ret;
}

static int
vtest_bo_export_dmabuf(struct vdrm_device *vdev, uint32_t handle)
{
   struct vtest_device *vtdev = to_vtest_device(vdev);
   uint32_t vtest_hdr[VTEST_HDR_SIZE];
   uint32_t vcmd_resource_export_fd[VCMD_RESOURCE_EXPORT_FD_SIZE];

   MESA_TRACE_FUNC();

   simple_mtx_lock(&vtdev->lock);
   struct vtest_bo *vbo = handle2bo(vtdev, handle);

   vtest_hdr[VTEST_CMD_LEN] = VCMD_RESOURCE_EXPORT_FD_SIZE;
   vtest_hdr[VTEST_CMD_ID] = VCMD_RESOURCE_EXPORT_FD;

   vcmd_resource_export_fd[VCMD_RESOURCE_EXPORT_FD_RES_HANDLE] = vbo->res_id;

   vtest_write(vtdev, vtest_hdr, sizeof(vtest_hdr));
   vtest_write(vtdev, vcmd_resource_export_fd, sizeof(vcmd_resource_export_fd));

   vtest_read(vtdev, vtest_hdr, sizeof(vtest_hdr));
   assert(vtest_hdr[VTEST_CMD_LEN] == 0);
   assert(vtest_hdr[VTEST_CMD_ID] == VCMD_RESOURCE_EXPORT_FD);

   int ret = vtest_receive_fd(vtdev);
   simple_mtx_unlock(&vtdev->lock);

   return ret;
}

static void
vtest_bo_close(struct vdrm_device *vdev, uint32_t handle)
{
   struct vtest_device *vtdev = to_vtest_device(vdev);

   MESA_TRACE_FUNC();

   /* Flush any buffered commands first, so the detach_resource doesn't
    * overtake any buffered ccmd which references the resource:
    *
    * TODO maybe move this into core??
    */
   if (vdev->reqbuf_len) {
      simple_mtx_lock(&vdev->eb_lock);
      vtest_flush_locked(vdev, NULL);
      simple_mtx_unlock(&vdev->eb_lock);
   }

   simple_mtx_lock(&vtdev->lock);
   struct vtest_bo *vbo = handle2bo(vtdev, handle);

   close_res_id(vtdev, vbo->res_id);
   util_idalloc_free(&vtdev->bo_idx_allocator, handle2idx(handle));
   simple_mtx_unlock(&vtdev->lock);
}

static void
vtest_close(struct vdrm_device *vdev)
{
   struct vtest_device *vtdev = to_vtest_device(vdev);

   MESA_TRACE_FUNC();

   simple_mtx_lock(&vtdev->lock);
   close_res_id(vtdev, vtdev->shmem_res_id);
   simple_mtx_unlock(&vtdev->lock);

   util_dynarray_fini(&vtdev->bo_table);
   util_idalloc_fini(&vtdev->bo_idx_allocator);

   close(vtdev->sock_fd);
}

static const struct vdrm_device_funcs funcs = {
   .flush_locked = vtest_flush_locked,
   .wait_fence = vtest_wait_fence,
   .execbuf_locked = vtest_execbuf_locked,
   .dmabuf_to_handle = vtest_dmabuf_to_handle,
   .handle_to_res_id = vtest_handle_to_res_id,
   .bo_create = vtest_bo_create,
   .bo_wait = vtest_bo_wait,
   .bo_map = vtest_bo_map,
   .bo_export_dmabuf = vtest_bo_export_dmabuf,
   .bo_close = vtest_bo_close,
   .close = vtest_close,
};

static int
connect_sock(void)
{
   struct sockaddr_un un;
   int sock, ret;

   MESA_TRACE_FUNC();

   sock = socket(PF_UNIX, SOCK_STREAM, 0);
   if (sock < 0)
      return -1;

   memset(&un, 0, sizeof(un));
   un.sun_family = AF_UNIX;
   snprintf(un.sun_path, sizeof(un.sun_path), "%s", VTEST_DEFAULT_SOCKET_NAME);

   do {
      ret = 0;
      if (connect(sock, (struct sockaddr *)&un, sizeof(un)) < 0) {
         ret = -errno;
      }
   } while (ret == -EINTR);

   if (ret) {
      close(sock);
      return ret;
   }

   return sock;
}

static void
send_init(struct vtest_device *vtdev)
{
   uint32_t buf[VTEST_HDR_SIZE];
   const char *comm = util_get_process_name();

   buf[VTEST_CMD_LEN] = strlen(comm) + 1;
   buf[VTEST_CMD_ID] = VCMD_CREATE_RENDERER;

   vtest_write(vtdev, &buf, sizeof(buf));
   vtest_write(vtdev, (void *)comm, strlen(comm) + 1);
}

static int
negotiate_version(struct vtest_device *vtdev)
{
   uint32_t vtest_hdr[VTEST_HDR_SIZE];
   uint32_t version_buf[VCMD_PROTOCOL_VERSION_SIZE];
   uint32_t busy_wait_buf[VCMD_BUSY_WAIT_SIZE];
   uint32_t busy_wait_result[1];
   ASSERTED int ret;

   vtest_hdr[VTEST_CMD_LEN] = VCMD_PING_PROTOCOL_VERSION_SIZE;
   vtest_hdr[VTEST_CMD_ID] = VCMD_PING_PROTOCOL_VERSION;
   vtest_write(vtdev, &vtest_hdr, sizeof(vtest_hdr));

   vtest_hdr[VTEST_CMD_LEN] = VCMD_BUSY_WAIT_SIZE;
   vtest_hdr[VTEST_CMD_ID] = VCMD_RESOURCE_BUSY_WAIT;
   busy_wait_buf[VCMD_BUSY_WAIT_HANDLE] = 0;
   busy_wait_buf[VCMD_BUSY_WAIT_FLAGS] = 0;
   vtest_write(vtdev, &vtest_hdr, sizeof(vtest_hdr));
   vtest_write(vtdev, &busy_wait_buf, sizeof(busy_wait_buf));

   ret = vtest_read(vtdev, vtest_hdr, sizeof(vtest_hdr));
   assert(ret);

   if (vtest_hdr[VTEST_CMD_ID] == VCMD_PING_PROTOCOL_VERSION) {
     /* Read dummy busy_wait response */
     ret = vtest_read(vtdev, vtest_hdr, sizeof(vtest_hdr));
     assert(ret);
     ret = vtest_read(vtdev, busy_wait_result, sizeof(busy_wait_result));
     assert(ret);

     vtest_hdr[VTEST_CMD_LEN] = VCMD_PROTOCOL_VERSION_SIZE;
     vtest_hdr[VTEST_CMD_ID] = VCMD_PROTOCOL_VERSION;
     version_buf[VCMD_PROTOCOL_VERSION_VERSION] = VTEST_PROTOCOL_VERSION;
     vtest_write(vtdev, &vtest_hdr, sizeof(vtest_hdr));
     vtest_write(vtdev, &version_buf, sizeof(version_buf));

     ret = vtest_read(vtdev, vtest_hdr, sizeof(vtest_hdr));
     assert(ret);
     ret = vtest_read(vtdev, version_buf, sizeof(version_buf));
     assert(ret);
     return version_buf[VCMD_PROTOCOL_VERSION_VERSION];
   }

   /* Read dummy busy_wait response */
   assert(vtest_hdr[VTEST_CMD_ID] == VCMD_RESOURCE_BUSY_WAIT);
   ret = vtest_read(vtdev, busy_wait_result, sizeof(busy_wait_result));
   assert(ret);

   /* Old server, return version 0 */
   return 0;
}

static int
get_capset(struct vtest_device *v)
{
   uint32_t vtest_hdr[VTEST_HDR_SIZE];
   uint32_t vcmd_get_capset[VCMD_GET_CAPSET_SIZE];

   vtest_hdr[VTEST_CMD_LEN] = VCMD_GET_CAPSET_SIZE;
   vtest_hdr[VTEST_CMD_ID] = VCMD_GET_CAPSET;

   vcmd_get_capset[VCMD_GET_CAPSET_ID] = VIRGL_RENDERER_CAPSET_DRM;
   vcmd_get_capset[VCMD_GET_CAPSET_VERSION] = 0;

   vtest_write(v, vtest_hdr, sizeof(vtest_hdr));
   vtest_write(v, vcmd_get_capset, sizeof(vcmd_get_capset));

   vtest_read(v, vtest_hdr, sizeof(vtest_hdr));
   assert(vtest_hdr[VTEST_CMD_ID] == VCMD_GET_CAPSET);

   uint32_t valid;
   vtest_read(v, &valid, sizeof(valid));
   if (!valid) {
      /* unsupported id or version */
      return -EINVAL;
   }

   size_t read_size = (vtest_hdr[VTEST_CMD_LEN] - 1) * 4;
   size_t capset_size = sizeof(v->base.caps);
   void *capset = &v->base.caps;
   if (capset_size >= read_size) {
      vtest_read(v, &v->base.caps, read_size);
      memset(capset + read_size, 0, capset_size - read_size);
   } else {
      vtest_read(v, capset, capset_size);

      /* If the remote end sends us back a larger capset, we need to
       * swallow the extra data:
       */
      char temp[256];
      read_size -= capset_size;
      while (read_size) {
         const size_t temp_size = MIN2(read_size, ARRAY_SIZE(temp));
         vtest_read(v, temp, temp_size);
         read_size -= temp_size;
      }
   }

   return 0;
}

static void
set_context(struct vtest_device *vtdev)
{
   uint32_t vtest_hdr[VTEST_HDR_SIZE];
   uint32_t vcmd_context_init[VCMD_CONTEXT_INIT_SIZE];

   vtest_hdr[VTEST_CMD_LEN] = VCMD_CONTEXT_INIT_SIZE;
   vtest_hdr[VTEST_CMD_ID] = VCMD_CONTEXT_INIT;
   vcmd_context_init[VCMD_CONTEXT_INIT_CAPSET_ID] = VIRGL_RENDERER_CAPSET_DRM;

   vtest_write(vtdev, vtest_hdr, sizeof(vtest_hdr));
   vtest_write(vtdev, vcmd_context_init, sizeof(vcmd_context_init));
}

static int
init_shmem(struct vtest_device *vtdev)
{
   struct vdrm_device *vdev = &vtdev->base;
   uint32_t res_id;
   int shmem_fd;

   res_id = vtest_create_blob(vtdev, SHMEM_SZ, VCMD_BLOB_FLAG_MAPPABLE,
                              0, &shmem_fd);
   if (!res_id)
      return -ENOMEM;

   vtdev->shmem_res_id = res_id;

   vdev->shmem = mmap(0, SHMEM_SZ, PROT_READ | PROT_WRITE,
                      MAP_SHARED, shmem_fd, 0);

   /* We no longer need the fd: */
   close(shmem_fd);

   if (!vdev->shmem) {
      close_res_id(vtdev, vtdev->shmem_res_id);
      vtdev->shmem_res_id = 0;
      return -ENOMEM;
   }

   uint32_t offset = vdev->shmem->rsp_mem_offset;
   vdev->rsp_mem_len = SHMEM_SZ - offset;
   vdev->rsp_mem = &((uint8_t *)vdev->shmem)[offset];

   return 0;
}

struct vdrm_device * vdrm_vtest_connect(int fd, uint32_t context_type);

struct vdrm_device *
vdrm_vtest_connect(int fd, uint32_t context_type)
{
   struct vtest_device *vtdev;
   struct vdrm_device *vdev = NULL;
   int sock_fd = connect_sock();
   int ret;

   MESA_TRACE_FUNC();

   if (sock_fd < 0) {
      mesa_loge("failed to connect: %s", strerror(errno));
      return NULL;
   }

   vtdev = calloc(1, sizeof(*vtdev));

   vtdev->sock_fd = sock_fd;
   simple_mtx_init(&vtdev->lock, mtx_plain);

   util_idalloc_init(&vtdev->bo_idx_allocator, 512);
   util_dynarray_init(&vtdev->bo_table, NULL);

   simple_mtx_lock(&vtdev->lock);
   send_init(vtdev);
   vtdev->protocol_version = negotiate_version(vtdev);

   /* Version 1 is deprecated. */
   if (vtdev->protocol_version == 1)
      vtdev->protocol_version = 0;

   // TODO minimum protocol version?

   mesa_logi("vtest connected, protocol version %d", vtdev->protocol_version);

   vdev = &vtdev->base;
   vdev->funcs = &funcs;

   ret = get_capset(vtdev);
   if (ret) {
      mesa_logi("could not get caps: %s", strerror(errno));
      goto error;
   }

   if (vdev->caps.context_type != context_type) {
      mesa_logi("wrong context_type: %u", vdev->caps.context_type);
      goto error;
   }

   set_context(vtdev);
   init_shmem(vtdev);

   simple_mtx_unlock(&vtdev->lock);

   return vdev;

error:
   simple_mtx_unlock(&vtdev->lock);
   vtest_close(vdev);
   return NULL;
}

/*
 * Sync-obj shim implementation, deals with us not having direct access to
 * syncobj related drm ioctls
 */
struct vtest_sync_provider {
   struct util_sync_provider base;
   struct vtest_device *vtdev;
};
DEFINE_CAST(util_sync_provider, vtest_sync_provider)

static int
vtest_drm_sync_create(struct util_sync_provider *p, uint32_t flags, uint32_t *handle)
{
   struct vtest_sync_provider *vp = to_vtest_sync_provider(p);
   uint32_t vtest_hdr[VTEST_HDR_SIZE];
   uint32_t vcmd_drm_sync_create[VCMD_DRM_SYNC_CREATE_SIZE];

   MESA_TRACE_FUNC();

   vtest_hdr[VTEST_CMD_LEN] = VCMD_DRM_SYNC_CREATE_SIZE;
   vtest_hdr[VTEST_CMD_ID] = VCMD_DRM_SYNC_CREATE;

   vcmd_drm_sync_create[VCMD_DRM_SYNC_CREATE_FLAGS] = flags;

   simple_mtx_lock(&vp->vtdev->lock);

   vtest_write(vp->vtdev, vtest_hdr, sizeof(vtest_hdr));
   vtest_write(vp->vtdev, vcmd_drm_sync_create, sizeof(vcmd_drm_sync_create));

   vtest_read(vp->vtdev, vtest_hdr, sizeof(vtest_hdr));
   assert(vtest_hdr[VTEST_CMD_LEN] == 1);
   assert(vtest_hdr[VTEST_CMD_ID] == VCMD_DRM_SYNC_CREATE);

   vtest_read(vp->vtdev, handle, sizeof(*handle));

   simple_mtx_unlock(&vp->vtdev->lock);

   return 0;
}

static int
vtest_drm_sync_destroy(struct util_sync_provider *p, uint32_t handle)
{
   struct vtest_sync_provider *vp = to_vtest_sync_provider(p);
   uint32_t vtest_hdr[VTEST_HDR_SIZE];
   uint32_t vcmd_drm_sync_destroy[VCMD_DRM_SYNC_DESTROY_SIZE];

   MESA_TRACE_FUNC();

   vtest_hdr[VTEST_CMD_LEN] = VCMD_DRM_SYNC_DESTROY_SIZE;
   vtest_hdr[VTEST_CMD_ID] = VCMD_DRM_SYNC_DESTROY;
   vcmd_drm_sync_destroy[VCMD_DRM_SYNC_DESTROY_HANDLE] = handle;

   simple_mtx_lock(&vp->vtdev->lock);

   vtest_write(vp->vtdev, vtest_hdr, sizeof(vtest_hdr));
   vtest_write(vp->vtdev, vcmd_drm_sync_destroy, sizeof(vcmd_drm_sync_destroy));

   simple_mtx_unlock(&vp->vtdev->lock);

   return 0;
}

static int
vtest_drm_sync_handle_to_fd(struct util_sync_provider *p, uint32_t handle, int *out_obj_fd)
{
   struct vtest_sync_provider *vp = to_vtest_sync_provider(p);
   uint32_t vtest_hdr[VTEST_HDR_SIZE];
   uint32_t vcmd_drm_sync_handle_to_fd[VCMD_DRM_SYNC_HANDLE_TO_FD_SIZE];
   int fd;

   MESA_TRACE_FUNC();

   vtest_hdr[VTEST_CMD_LEN] = VCMD_DRM_SYNC_HANDLE_TO_FD_SIZE;
   vtest_hdr[VTEST_CMD_ID] = VCMD_DRM_SYNC_HANDLE_TO_FD;
   vcmd_drm_sync_handle_to_fd[VCMD_DRM_SYNC_HANDLE_TO_FD_HANDLE] = handle;

   simple_mtx_lock(&vp->vtdev->lock);

   vtest_write(vp->vtdev, vtest_hdr, sizeof(vtest_hdr));
   vtest_write(vp->vtdev, vcmd_drm_sync_handle_to_fd, sizeof(vcmd_drm_sync_handle_to_fd));

   vtest_read(vp->vtdev, vtest_hdr, sizeof(vtest_hdr));
   assert(vtest_hdr[VTEST_CMD_LEN] == 0);
   assert(vtest_hdr[VTEST_CMD_ID] == VCMD_DRM_SYNC_HANDLE_TO_FD);

   fd = vtest_receive_fd(vp->vtdev);

   simple_mtx_unlock(&vp->vtdev->lock);

   if (fd < 0)
      return fd;

   *out_obj_fd = fd;

   return 0;
}

static int
vtest_drm_sync_fd_to_handle(struct util_sync_provider *p, int obj_fd, uint32_t *out_handle)
{
   struct vtest_sync_provider *vp = to_vtest_sync_provider(p);
   uint32_t vtest_hdr[VTEST_HDR_SIZE];
   uint32_t handle;

   MESA_TRACE_FUNC();

   vtest_hdr[VTEST_CMD_LEN] = VCMD_DRM_SYNC_FD_TO_HANDLE_SIZE;
   vtest_hdr[VTEST_CMD_ID] = VCMD_DRM_SYNC_FD_TO_HANDLE;

   simple_mtx_lock(&vp->vtdev->lock);

   vtest_write(vp->vtdev, vtest_hdr, sizeof(vtest_hdr));

   vtest_send_fd(vp->vtdev, obj_fd);

   vtest_read(vp->vtdev, vtest_hdr, sizeof(vtest_hdr));
   assert(vtest_hdr[VTEST_CMD_LEN] == 1);
   assert(vtest_hdr[VTEST_CMD_ID] == VCMD_DRM_SYNC_HANDLE_TO_FD);

   vtest_read(vp->vtdev, &handle, sizeof(handle));

   simple_mtx_unlock(&vp->vtdev->lock);

   *out_handle = handle;

   return 0;
}

static int
vtest_drm_sync_import_sync_file(struct util_sync_provider *p, uint32_t handle, int sync_file_fd)
{
   struct vtest_sync_provider *vp = to_vtest_sync_provider(p);
   uint32_t vtest_hdr[VTEST_HDR_SIZE];
   uint32_t vcmd_drm_sync_import_sync_file[VCMD_DRM_SYNC_IMPORT_SYNC_FILE_SIZE];

   MESA_TRACE_FUNC();

   vtest_hdr[VTEST_CMD_LEN] = VCMD_DRM_SYNC_IMPORT_SYNC_FILE_SIZE;
   vtest_hdr[VTEST_CMD_ID] = VCMD_DRM_SYNC_IMPORT_SYNC_FILE;
   vcmd_drm_sync_import_sync_file[VCMD_DRM_SYNC_IMPORT_SYNC_FILE_HANDLE] = handle;

   simple_mtx_lock(&vp->vtdev->lock);

   vtest_write(vp->vtdev, vtest_hdr, sizeof(vtest_hdr));
   vtest_write(vp->vtdev, vcmd_drm_sync_import_sync_file, sizeof(vcmd_drm_sync_import_sync_file));

   vtest_send_fd(vp->vtdev, sync_file_fd);

   simple_mtx_unlock(&vp->vtdev->lock);

   return 0;
}

static int
vtest_drm_sync_export_sync_file(struct util_sync_provider *p, uint32_t handle, int *out_sync_file_fd)
{
   struct vtest_sync_provider *vp = to_vtest_sync_provider(p);
   uint32_t vtest_hdr[VTEST_HDR_SIZE];
   uint32_t vcmd_drm_sync_export_sync_file[VCMD_DRM_SYNC_EXPORT_SYNC_FILE_SIZE];
   int fd;

   MESA_TRACE_FUNC();

   vtest_hdr[VTEST_CMD_LEN] = VCMD_DRM_SYNC_EXPORT_SYNC_FILE_SIZE;
   vtest_hdr[VTEST_CMD_ID] = VCMD_DRM_SYNC_EXPORT_SYNC_FILE;
   vcmd_drm_sync_export_sync_file[VCMD_DRM_SYNC_EXPORT_SYNC_FILE_HANDLE] = handle;

   simple_mtx_lock(&vp->vtdev->lock);

   vtest_write(vp->vtdev, vtest_hdr, sizeof(vtest_hdr));
   vtest_write(vp->vtdev, vcmd_drm_sync_export_sync_file, sizeof(vcmd_drm_sync_export_sync_file));

   vtest_read(vp->vtdev, vtest_hdr, sizeof(vtest_hdr));
   assert(vtest_hdr[VTEST_CMD_LEN] == 0);
   assert(vtest_hdr[VTEST_CMD_ID] == VCMD_DRM_SYNC_EXPORT_SYNC_FILE);

   fd = vtest_receive_fd(vp->vtdev);

   simple_mtx_unlock(&vp->vtdev->lock);

   if (fd < 0)
      return fd;

   *out_sync_file_fd = fd;

   return 0;
}

static int
vtest_drm_sync_wait(struct util_sync_provider *p, uint32_t *handles, unsigned num_handles,
                int64_t timeout_nsec, unsigned flags, uint32_t *first_signaled)
{
   struct vtest_sync_provider *vp = to_vtest_sync_provider(p);
   uint32_t vtest_hdr[VTEST_HDR_SIZE];
   uint32_t vcmd_drm_sync_wait[VCMD_DRM_SYNC_WAIT_SIZE];
   int32_t ret;
   int wait_fd = -1;

   MESA_TRACE_FUNC();

   if (timeout_nsec > os_time_get_nano())
      flags |= DRM_SYNCOBJ_WAIT_FLAGS_WAIT_FD;

   vtest_hdr[VTEST_CMD_LEN] = VCMD_DRM_SYNC_WAIT_SIZE;
   vtest_hdr[VTEST_CMD_ID] = VCMD_DRM_SYNC_WAIT;
   vcmd_drm_sync_wait[VCMD_DRM_SYNC_WAIT_NUM_HANDLES] = num_handles;
   vcmd_drm_sync_wait[VCMD_DRM_SYNC_WAIT_TIMEOUT_LO] = (uint32_t)timeout_nsec;
   vcmd_drm_sync_wait[VCMD_DRM_SYNC_WAIT_TIMEOUT_HI] = (uint32_t)(timeout_nsec >> 32);
   vcmd_drm_sync_wait[VCMD_DRM_SYNC_WAIT_FLAGS] = flags;

   simple_mtx_lock(&vp->vtdev->lock);

   vtest_write(vp->vtdev, vtest_hdr, sizeof(vtest_hdr));
   vtest_write(vp->vtdev, vcmd_drm_sync_wait, sizeof(vcmd_drm_sync_wait));
   vtest_write(vp->vtdev, handles, num_handles * sizeof(uint32_t));

   vtest_read(vp->vtdev, vtest_hdr, sizeof(vtest_hdr));
   assert(vtest_hdr[VTEST_CMD_ID] == VCMD_DRM_SYNC_WAIT);

   if (flags & DRM_SYNCOBJ_WAIT_FLAGS_WAIT_FD) {
      assert(vtest_hdr[VTEST_CMD_LEN] == 0);
      wait_fd = vtest_receive_fd(vp->vtdev);
   } else {
      assert(vtest_hdr[VTEST_CMD_LEN] == 2);

      vtest_read(vp->vtdev, &ret, sizeof(ret));
      if (first_signaled)
         *first_signaled = ret;
      vtest_read(vp->vtdev, &ret, sizeof(ret));
   }

   simple_mtx_unlock(&vp->vtdev->lock);

   if (flags & DRM_SYNCOBJ_WAIT_FLAGS_WAIT_FD) {
      vtest_read_fd(wait_fd, &ret, sizeof(ret));
      if (first_signaled)
         *first_signaled = ret;
      vtest_read_fd(wait_fd, &ret, sizeof(ret));

      close(wait_fd);
   }

   if (ret)
      errno = -ret;

   return ret;
}

static int
vtest_drm_sync_reset(struct util_sync_provider *p, const uint32_t *handles, uint32_t num_handles)
{
   struct vtest_sync_provider *vp = to_vtest_sync_provider(p);
   uint32_t vtest_hdr[VTEST_HDR_SIZE];
   uint32_t vcmd_drm_sync_reset[VCMD_DRM_SYNC_RESET_SIZE];

   MESA_TRACE_FUNC();

   vtest_hdr[VTEST_CMD_LEN] = VCMD_DRM_SYNC_RESET_SIZE;
   vtest_hdr[VTEST_CMD_ID] = VCMD_DRM_SYNC_RESET;
   vcmd_drm_sync_reset[VCMD_DRM_SYNC_WAIT_NUM_HANDLES] = num_handles;

   simple_mtx_lock(&vp->vtdev->lock);

   vtest_write(vp->vtdev, vtest_hdr, sizeof(vtest_hdr));
   vtest_write(vp->vtdev, vcmd_drm_sync_reset, sizeof(vcmd_drm_sync_reset));
   vtest_write(vp->vtdev, handles, num_handles * sizeof(uint32_t));

   simple_mtx_unlock(&vp->vtdev->lock);

   return 0;
}

static int
vtest_drm_sync_signal(struct util_sync_provider *p, const uint32_t *handles, uint32_t num_handles)
{
   struct vtest_sync_provider *vp = to_vtest_sync_provider(p);
   uint32_t vtest_hdr[VTEST_HDR_SIZE];
   uint32_t vcmd_drm_sync_signal[VCMD_DRM_SYNC_SIGNAL_SIZE];

   MESA_TRACE_FUNC();

   vtest_hdr[VTEST_CMD_LEN] = VCMD_DRM_SYNC_SIGNAL_SIZE;
   vtest_hdr[VTEST_CMD_ID] = VCMD_DRM_SYNC_SIGNAL;
   vcmd_drm_sync_signal[VCMD_DRM_SYNC_SIGNAL_NUM_HANDLES] = num_handles;

   simple_mtx_lock(&vp->vtdev->lock);

   vtest_write(vp->vtdev, vtest_hdr, sizeof(vtest_hdr));
   vtest_write(vp->vtdev, vcmd_drm_sync_signal, sizeof(vcmd_drm_sync_signal));
   vtest_write(vp->vtdev, handles, num_handles * sizeof(uint32_t));

   simple_mtx_unlock(&vp->vtdev->lock);

   return 0;
}

static int
vtest_drm_sync_query(struct util_sync_provider *p, uint32_t *handles, uint64_t *points,
                 uint32_t num_handles, uint32_t flags)
{
   struct vtest_sync_provider *vp = to_vtest_sync_provider(p);
   uint32_t vtest_hdr[VTEST_HDR_SIZE];
   uint32_t vcmd_drm_sync_query[VCMD_DRM_SYNC_QUERY_SIZE];

   MESA_TRACE_FUNC();

   vtest_hdr[VTEST_CMD_LEN] = VCMD_DRM_SYNC_QUERY_SIZE;
   vtest_hdr[VTEST_CMD_ID] = VCMD_DRM_SYNC_QUERY;
   vcmd_drm_sync_query[VCMD_DRM_SYNC_QUERY_NUM_HANDLES] = num_handles;
   vcmd_drm_sync_query[VCMD_DRM_SYNC_QUERY_NUM_FLAGS] = flags;

   simple_mtx_lock(&vp->vtdev->lock);

   vtest_write(vp->vtdev, vtest_hdr, sizeof(vtest_hdr));
   vtest_write(vp->vtdev, vcmd_drm_sync_query, sizeof(vcmd_drm_sync_query));
   vtest_write(vp->vtdev, handles, num_handles * sizeof(uint32_t));

   vtest_read(vp->vtdev, vtest_hdr, sizeof(vtest_hdr));
   assert(vtest_hdr[VTEST_CMD_LEN] == 0);
   assert(vtest_hdr[VTEST_CMD_ID] == VCMD_DRM_SYNC_QUERY);

   vtest_read(vp->vtdev, points, num_handles * sizeof(uint64_t));

   simple_mtx_unlock(&vp->vtdev->lock);

   return 0;
}

static int
vtest_drm_sync_transfer(struct util_sync_provider *p, uint32_t dst_handle, uint64_t dst_point,
                    uint32_t src_handle, uint64_t src_point, uint32_t flags)
{
   struct vtest_sync_provider *vp = to_vtest_sync_provider(p);
   uint32_t vtest_hdr[VTEST_HDR_SIZE];
   uint32_t vcmd_drm_sync_transfer[VCMD_DRM_SYNC_TRANSFER_SIZE];

   MESA_TRACE_FUNC();

   vtest_hdr[VTEST_CMD_LEN] = VCMD_DRM_SYNC_TRANSFER_SIZE;
   vtest_hdr[VTEST_CMD_ID] = VCMD_DRM_SYNC_TRANSFER;
   vcmd_drm_sync_transfer[VCMD_DRM_SYNC_TRANSFER_DST_HANDLE] = dst_handle;
   vcmd_drm_sync_transfer[VCMD_DRM_SYNC_TRANSFER_DST_POINT_LO] = (uint32_t)dst_point;
   vcmd_drm_sync_transfer[VCMD_DRM_SYNC_TRANSFER_DST_POINT_HI] = (uint32_t)(dst_point >> 32);
   vcmd_drm_sync_transfer[VCMD_DRM_SYNC_TRANSFER_SRC_HANDLE] = src_handle;
   vcmd_drm_sync_transfer[VCMD_DRM_SYNC_TRANSFER_SRC_POINT_LO] = (uint32_t)src_point;
   vcmd_drm_sync_transfer[VCMD_DRM_SYNC_TRANSFER_SRC_POINT_HI] = (uint32_t)(src_point >> 32);
   vcmd_drm_sync_transfer[VCMD_DRM_SYNC_TRANSFER_FLAGS] = flags;

   simple_mtx_lock(&vp->vtdev->lock);

   vtest_write(vp->vtdev, vtest_hdr, sizeof(vtest_hdr));
   vtest_write(vp->vtdev, vcmd_drm_sync_transfer, sizeof(vcmd_drm_sync_transfer));

   simple_mtx_unlock(&vp->vtdev->lock);

   return 0;
}

static void
vtest_sync_finalize(struct util_sync_provider *p)
{
   free(p);
}

struct util_sync_provider *
vdrm_vtest_get_sync(struct vdrm_device *vdrm)
{
   if (vdrm->funcs != &funcs)
      return NULL;

   struct vtest_sync_provider *s = calloc(1, sizeof(*s));

   s->base = (struct util_sync_provider){
      .create = vtest_drm_sync_create,
      .destroy = vtest_drm_sync_destroy,
      .handle_to_fd = vtest_drm_sync_handle_to_fd,
      .fd_to_handle = vtest_drm_sync_fd_to_handle,
      .import_sync_file = vtest_drm_sync_import_sync_file,
      .export_sync_file = vtest_drm_sync_export_sync_file,
      .wait = vtest_drm_sync_wait,
      .reset = vtest_drm_sync_reset,
      .signal = vtest_drm_sync_signal,
      .query = vtest_drm_sync_query,
      .transfer = vtest_drm_sync_transfer,
      .finalize = vtest_sync_finalize,
   };
   s->vtdev = to_vtest_device(vdrm);

   return &s->base;
}
