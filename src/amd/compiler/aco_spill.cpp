            ...
            for (unsigned t : ctx.program->live.live_in[block_idx]) {
               ...
               if (can_rematerialize > do_rematerialize || loop_variable > avoid_respill ||
                  ...
                  for (auto it = instr->operands.begin(); it != instr->operands.end(); ++it) {
                     ...
                     if (!instr->isCall() &&
                         ((type == RegType::sgpr && live_changes->sgpr < (int16_t)it->size()) ||
                          (type == RegType::vgpr && live_changes->vgpr < (int16_t)it->size()))) {
                        can_spill = false;
                        break;
                     }
                     if (instr->isCall() && !it->isClobbered()) {
                        can_spill = false;
                        break;
                     }
...
spill(Program* program)
{
   ...
   if (program->has_call) {
      ...
      if (!call_needs_spill)
         return;
   } else if (program->num_waves > 0 && program->cur_reg_demand.sgpr <= abi_sgpr_limit) {
      return;
   }
   ...
   if (demand.sgpr > abi_sgpr_limit)
      sgpr_spills += demand.sgpr - abi_sgpr_limit;
   ...
   if (demand.sgpr > abi_sgpr_limit && ctx.program->is_callee) {
      ...
      for (PhysReg reg : ctx.program->callee_abi.preservedRegisters(max_reg, vgpr_limit))
