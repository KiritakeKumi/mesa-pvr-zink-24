...
ANDROID_CTS_TESTS_KEEP=(
  "CtsGraphicsTestCases"
)

ANDROID_CTS_TESTS_KEEP_EXPRESSION=$(printf "%s|" "${ANDROID_CTS_TESTS_KEEP[@]}" | sed -e 's/|$//g')
find android-cts/testcases/ -mindepth 1 -type d | grep -v -E "$ANDROID_CTS_TESTS_KEEP_EXPRESSION" | xargs rm -rf
