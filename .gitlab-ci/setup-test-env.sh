...
# ------ Structured tagging
export _CI_TAG_CHECK_FILE=".mesa-ci-build-tag"
...
_ci_tag_check() (
    ...
    if [ -z "${_declared:-}" ]; then
        ...
        echo "bin/ci/update_tag.sh --component ${component_lower}"
...
ci_tag_build_time_check() {
    ...
    local extra_files=("${@:2}")
    local calculated_tag=$(_ci_calculate_tag "${build_script}" "${extra_files[@]}")
